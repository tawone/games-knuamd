...
function BottomNav({ active }: { active: string }) {
  const tabs = [
    { id: 'home', icon: '🏠', label: 'Home', href: 'https://knuamd.com' },
    { id: 'library', icon: '📚', label: 'Library', href: 'https://read.knuamd.com' },
    { id: 'courses', icon: '📖', label: 'Courses', href: 'https://courses.knuamd.com' },
    { id: 'games', icon: '🎮', label: 'Games', href: 'https://games.knuamd.com' },
    { id: 'ai', icon: '🤖', label: 'AI', href: 'https://ai.knuamd.com' },
  ]
        ...
        <div className="flex items-center justify-around py-2 pb-[max(0.5rem,env(safe-area-inset-bottom))]">
          {tabs.map(tab => (
            <a
              ...
              href={tab.href}
              className={cn(
                'flex flex-col items-center gap-0.5 px-3 py-1 rounded-lg transition-all min-w-[52px] no-underline',
                tab.id === active ? 'text-orange-400' : 'text-gray-400 hover:text-gray-600'
              )}
            >
...
export default function App() {
  ...
  if (screen === 'fun') return <div className="frame"><FunMenu onSelect={(s) => setScreen(s)} onBack={() => setScreen('menu')} /></div>
  if (screen === 'match') return <div className="frame"><WordMatch onBack={() => setScreen('fun')} /></div>
  if (screen === 'quiz') return <div className="frame"><WordQuiz onBack={() => setScreen('fun')} /></div>
  if (screen === 'fill') return <div className="frame"><FillBlank onBack={() => setScreen('fun')} /></div>
  if (screen === 'talks') return <div className="frame"><Talks onBack={() => setScreen('menu')} /></div>
  if (screen === 'novel') return <div className="frame"><InteractiveNovel onBack={() => setScreen('menu')} /></div>
  if (screen === 'admin-novels') return <div className="frame"><NovelsAdmin onBack={() => setScreen('menu')} /></div>

  return (
    <div className="min-h-screen bg-[#FDF6EE] flex items-start justify-center">
      <div className="relative w-full max-w-[min(430px,85vw)] min-h-screen bg-[#FDF6EE] overflow-y-auto overflow-x-hidden pb-16 isolation isolate max-lg:rounded-none lg:rounded-3xl lg:my-4 lg:min-h-[calc(100vh-2rem)] lg:shadow-2xl lg:border lg:border-orange-100">
        <div className="p-4">
